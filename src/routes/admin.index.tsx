...
import { Users, FolderKanban, ListChecks, CheckCircle2, TrendingUp, CalendarCheck, Loader2 } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        <StatCard icon={Users} label="Total Interns" value={dashboardStats.totalInterns.toString()} delta="12%" tone="primary" />
        <StatCard icon={FolderKanban} label="Active Projects" value={dashboardStats.activeProjects.toString()} delta="4%" tone="secondary" />
        <StatCard icon={ListChecks} label="Pending Tasks" value={dashboardStats.pendingTasks.toString()} delta="8%" trend="down" tone="warning" />
        <StatCard icon={CheckCircle2} label="Completed" value={dashboardStats.completedTasks.toLocaleString()} delta="22%" tone="success" />
        <StatCard icon={TrendingUp} label="Productivity" value={`${dashboardStats.productivity}%`} delta="3%" tone="primary" />
        <StatCard icon={CalendarCheck} label="Attendance" value={`${dashboardStats.attendance}%`} delta="1%" tone="success" />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="p-6 rounded-2xl glass shadow-soft">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <RadialBarChart innerRadius="70%" outerRadius="100%" data={[{ name: "score", v: dashboardStats.productivity, fill: "oklch(0.51 0.23 277)" }]} startAngle={90} endAngle={-270}>
          ...
          <div className="grid grid-cols-3 gap-2 mt-16 text-center text-xs">
            ...
            <div><div className="font-bold">{getGrade(dashboardStats.productivity)}</div><div className="text-muted-foreground">Grade</div></div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="p-6 rounded-2xl glass shadow-soft">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-primary font-medium cursor-pointer" onClick={() => navigate({ to: "/admin/interns", search: { add: undefined } })}>View all</span>
          </div>
          <div className="space-y-3">
            {dashboardStats.topPerformers.map((r: { n: string; d: string; s: number }, i: number) => (
              <div key={r.n} className="flex items-center gap-3">
                ...
                <div className="size-9 rounded-full bg-gradient-primary grid place-items-center text-primary-foreground text-xs font-semibold">{r.n.split(" ").map((s: string)=>s[0]).join("")}</div>
