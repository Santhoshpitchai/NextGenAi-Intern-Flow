...
export const Route = createFileRoute("/signup")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Create your InternFlow AI account in under a minute.",
      },
    ],
  }),
    ...
    <div className="min-h-screen bg-gradient-mesh px-4 py-8 sm:px-6 sm:py-12">
      <div
        className={cn(
          ...
          role === "intern" ? "max-w-5xl" : "max-w-2xl"
        )}
      >
        {/* Logo */}
        <div className="mb-6 text-center">
        ...
        <div className="rounded-3xl glass p-6 shadow-glass sm:p-8 md:p-10">
          ...
          {/* Role Toggle */}
          <div className="relative mt-8 grid grid-cols-2 rounded-xl bg-muted p-1" role="tablist" aria-label="Signup role selector">
            <div
              className={cn(
                "pointer-events-none absolute inset-y-1 left-1 w-[calc(50%-0.25rem)] rounded-lg bg-background shadow-soft transition-transform duration-300 ease-out",
                role === "intern" && "translate-x-full"
              )}
              aria-hidden
            />
            ...
            {ROLES.map((r) => (
              <button
                ...
                onClick={() => setRole(r.id)}
                className={cn(
                  "relative z-10 w-full rounded-lg px-3 text-sm font-semibold transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
                  role === r.id
                    ? "text-foreground"
