...
export const Route = createFileRoute("/admin/chat")({
  head: () => ({ meta: [{ title: "Team Chat — InternFlow AI" }] }),
  component: AdminChat,
});
...
function AdminChat() {
  ...
  // Query workspace assignments for composing briefings
  ...
  // Query all tasks for composing briefings
  const { data: allTasks } = useQuery({
    queryKey: ["all-tasks"],
    queryFn: () => taskApi.getAllTasks(),
  });

  const filteredTasks = allTasks ? allTasks.filter((t) => t.assignment?.id === selectedAssignmentId) : [];
  ...
  const handleSendDraft = () => {
    ...
    const selectedTask = allTasks?.find((t) => t.id === actualTaskId);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <PageHeader
        title="Admin Team Chat & Announcements"
        subtitle="Auditing communications, micro-managing loops, and drafting administrative directives."
      />
                    ...
                    >
                      ...
                      <div className={`flex flex-col max-w-[70%] ${isOwnMessage ? "items-end" : ""}`}>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {message.sender.role.includes("ADMIN") && (
                            <Badge variant="secondary" className="text-[8px] bg-primary/10 text-primary font-extrabold px-1.5 py-0.25">
                              Admin
                            </Badge>
                          )}
      ...
      <Dialog open={draftModalOpen} onOpenChange={setDraftModalOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Compose a formatted administrative announcement. Tagged workspace accounts will receive a notification loop binder.
            </DialogDescription>
          </DialogHeader>
